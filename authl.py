...
async def auth_processor():
    ...
    botspamchannel = interactions.Channel(
        **await bot._http.get_channel(bschan), _client=bot._http
    )
    ...
    while True:
        ...
        if results:
            ...
            for r in results:
                ...
                try:
                    user = interactions.Member(
                        **await bot._http.get_member(guildid, r[1]), _client=bot._http
                    )
                except Exception:
                    user = None
                ...
                if user is None:
                    logging.info(
                        f"User with discord id {r[1]} is not in the server, skipping."
                    )
                    err_params = {"userid": userid}
                    query(dbfile, err_query, err_params)
                    await botspamchannel.send(
                        f"User https://forums.somethingawful.com/member.php?action=getinfo&userid={userid} left discord, please unsus if they rejoin."
                    )
                    continue
                ...
                if position > -1:
                    ...
                    try:
                        ...
                        await botspamchannel.send(
                            f"Authenticating user {user.mention} failed."
                        )
                        await asyncio.sleep(10)
                        continue
                await asyncio.sleep(10)
            ...
            if displaymessage:
                await botspamchannel.send(
                    f"Gave goon role to the following users {success}"
                )
...
async def get_user(userid, discordid):
    ...
    if not results:
        ...
        ins_query = (
            """INSERT INTO goons values (:userid,:discordid,:secret,:ban,0,:sus)"""
        )
        ins_params = {
            "userid": userid,
            "discordid": discordid,
    ...
    if results[0][1] != discordid:
        ...
        raise DiscordMismatch(
            f"User provided {discordid}, db contained {results[0][1]}"
        )
...
async def authme(ctx: interactions.CommandContext, username: str):
    ...
    if results[0][5]:
        await botspamchannel.send(
            "User "
            + ctx.author.mention
            + " needs attention to complete authentication."
        )
        response = (
            response
...
async def authem(
    ctx: interactions.CommandContext, user: interactions.User, username: str
):
    ...
    try:
        ...
        response = (
            "An error occured: This discord account is registered to another SA user."
        )
        await ctx.send(response)
        return
        ...
        response = (
            "An error occured. This SA account is registered to another discord user."
        )
        await ctx.send(response)
        return
    ...
    response = (
        ...
        + "\n"
        + user.mention
        + f' Put the following key in your SA profile "about me" section: {results[0][2]}\nDo not put it in ICQ number or homepage or any other field.'
    )

    if results[0][5]:
        await botspamchannel.send(
            "User " + user.mention + " needs attention to complete authentication."
        )
        response = (
            response
            + "\n\nA member of leadership may contact you to complete your authentication."
...
async def listunauth(ctx: interactions.CommandContext):
    ...
    querystr = (
        """SELECT * FROM goons WHERE is_authed = 0 AND is_sus = 0 AND is_banned = 0"""
    )
...
async def bangoon(ctx: interactions.CommandContext, username: str):
    ...
    querystr = (
        """UPDATE goons SET is_banned = 1, is_authed=0 WHERE userID=:userid LIMIT 1"""
    )
...
async def main():
    ...
    foreground = asyncio.create_task(bot._ready())
        ...
        [background, foreground], return_when=asyncio.FIRST_EXCEPTION
    )
