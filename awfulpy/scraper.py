...
class AwfulScraper:
    ...
    async def fetch_posts_since_last_seen(self):
        ...
        self.logger.debug(
            "Old start_page: {0}, Old last_seen: {1}".format(pagenum, last_seen)
        )
        posts = []
        while True:
        ...
        self.logger.debug(
            "New start_page: {0}, New last_seen: {1}".format(pagenum, last_seen)
        )
    ...
    async def reply_to_thread(self, postbody):
        ...
        self.logger.debug(
            "Threadid: {0}, formkey: {1}, form_cookie: {2}".format(
                replyobj.threadid, replyobj.formkey, replyobj.form_cookie
            )
        )
        payload = {
            "action": "postreply",
        ...
        async with aiohttp.ClientSession(cookies=self.cookies) as session:
            async with session.post(
                do_post_url, data=payload, headers=headers
            ) as response:
                html = await response.text()
